...
//     ccc.open("get", "http://numbersapi.com/random?min=1&max=200"); // 1부터 200사이의 랜덤한 값을 뽑아주는 api 주소
//     ccc.send();
//     ccc.addEventListener("load", (response) => {
//       const num = response.target.response.split(" ")[0];
//       setMyCallback(num);

//     }); // 받아서 요청이 끝났을때 그때 실행될 나만의 로직을 여기다 적음
//     // await이 안되기 때문에 요청이 끝났을때 "load", 다음에 콜백함수를 넣어줬음
//   }

//   return (
//     <>
//       <h1>콜백과 친구들</h1>
//       <span>결과 : {myCallback}</span>
...
export default function CallbackPromiseAsyncAwaitPage() {
  ...
  function onClickMyCallback() {
    ...
    ccc.addEventListener("load", (res) => {
      ...
      aaa.addEventListener("load", (res) => {
        ...
        const user = JSON.parse(res.target.response).UsedId;
  ...
  } // Promise를 쓰면 콜백지옥을 해결 할 수 있다.
  // CreateBoard().then => 이런식으로 프로미스 체이닝이 가능함
